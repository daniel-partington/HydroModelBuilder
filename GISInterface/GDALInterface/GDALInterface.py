...
class GDALInterface(GISInterface):
    ...
    Functions currently implemented are:
        ...
        2. set_model_boundary_from_corners
        3. define_structured_mesh
        4. read_rasters
        5. map_rasters_to_grid        
        6. read_polyline
        7. map_polyline_to_grid
        8. 
        9. map_points_to_grid
    ...
    def set_data_boundary_from_polygon_shapefile(self, shapefile_name, shapefile_path=None, buffer_dist=None):
        ...
        if os.path.isfile(new_file):
            ...
            self.data_boundary = create_buffer.create_buffer4poly(shapefile_name, buffer_distance=buffer_dist)
        # End if    
    ...
    def define_structured_mesh(self, gridHeight, gridWidth):
        ...
        new_file = self.out_data_folder + 'structured_model_grid_%im' %int(gridHeight) + '\\structured_model_grid_%im.shp' %int(gridHeight)
        ...
        if os.path.isfile(new_file):
            ...
            self.model_mesh = fishnet.create_fishnet(structured_mesh, self.model_boundary[4], copy_dest=self.out_data_folder)
        # End if
        ...
        for raster in files:
            if os.path.isfile(self.out_data_folder + raster + '_reproj.bil'):
                ...
                if self.projected_coordinate_system == srs:
                    ...
                    command = 'gdalwarp -q -t_srs ' + target_srs + ' -r bilinear "' + path + raster + '" "' + self.out_data_folder + raster + '_reproj.bil"'
    ...
    def map_rasters_to_grid(self, raster_files, raster_path, raster_driver = None):
        ...
        if self.mesh_type == 'structured':
            new_files = [self.out_data_folder_grid + raster + '_model_grid.bil' for raster in raster_files]
            new_files_exist = [os.path.isfile(self.out_data_folder_grid + raster + '_model_grid.bil') for raster in raster_files]
            ...
            if False not in new_files_exist:            
                ...
                set_bounds = (xmin, xmin + cols*self.gridHeight + 1.0, ymax - rows*self.gridHeight, ymax)        
...
#                    break                
                ...
                for raster in raster_files:
                    ...
                    #FNULL = open(os.devnull, 'w')
                    command = "gdalwarp -t_srs " + target_srs + ' -cutline "' + self.boundary_poly_file + '" -crop_to_cutline "' + raster_path + raster + '" "' + self.out_data_folder + raster + '_clipped.bil"'
                    print command # -dstalpha 
                    subprocess.call(command)#, stdout=FNULL, stderr=subprocess.STDOUT)                
                    
                    # Use clipped raster to map active raster cells to grid
                    ds = gdal.Open(self.out_data_folder + raster + '_clipped.bil', gdalconst.GA_ReadOnly) # raster_path  
    ...
    def create_basement_bottom(self, hu_raster_path, surface_raster_file, basement_top_raster_file, basement_bot_raster_file, output_path, raster_driver=None):
        ...
        """
        basement.create_basement_bottom(hu_raster_path, surface_raster_file, basement_top_raster_file, basement_bot_raster_file, output_path, raster_driver=raster_driver)
    ...
    def read_polyline(self, filename, path=None):     
        ...
        if srs == self.projected_coordinate_system:        
            ...
            ds = reproject.reproject_layer(ds, 
                                           ...
                                           dst_cs=self.projected_coordinate_system, 
                                           create_copy=True,
                                           copy_dest=self.out_data_folder + filename[:-4]+'_model.shp',    
    ...
    def read_points_data(self, filename, path=None):
        ...
        if os.path.isfile(new_file):
            ...
            command = 'ogr2ogr -t_srs "' + target_srs + '" "'  + self.out_data_folder + filename.split('\\')[-1][:-4] + '_reproj.shp" "' + filename + '"'
            #print command            
            subprocess.call(command)
