...
python and modified as well to work with the GW_modelbuilder class.
"""
...
class PESTInterface(object):
    ...
    def genpestfiles(self, models_ID):     
        ...
        if strcmp(PESTcon.VALUE{'PESTMODE'},'prediction')
            if ~strcmp(PESTobs.OBGNME(:),'predict')
        ...
        for n = 1:size(models_ID,2)
            # Find model folder
            model_folder = ls([root_folder '\' models_ID{n} '_*']);
            if isempty(model_folder)
                error('Model folder not found for model %s', models_ID{n});
            elseif size(model_folder,1)>1
            ...
            OUTFLE{n} = ['..\' model_folder '\observations_' models_ID{n} '.txt'];
        ...
        NTPLFILE=length(TEMPFLE);
        NINSFLE =length(INSFLE);
        
        # Open file
        fid=fopen([PEST_folder '\' PESTFILE],'wt');
        fprintf(fid,'pcf\n');
        ...
        for i=1:size(PESTobsgp,1)
           fprintf(fid,'%s\n',PESTobsgp{i});
        end
        
        # Observations
        fprintf(fid,'* observation data\n');
        for i=1:size(PESTobs,1)
           fprintf(fid,'%s\t%g\t%g\t%s\n',...
        ...
        for i=1:length(TEMPFLE)
           fprintf(fid,'%s\t%s\n',TEMPFLE{i},INFLE{i});
        end
        for i=1:length(INSFLE)
           fprintf(fid,'%s\t%s\n',INSFLE{i},OUTFLE{i});
        end
        ...
        # Predictive analysis
        fprintf(fid,'* predictive analysis\n');
        fprintf(fid,'%d\n',str2double(PESTcon.VALUE{'NPREDMAXMIN'}));
        fprintf(fid,'%g %g %g\n',str2double(PESTcon.VALUE{'PD0'}),str2double(PESTcon.VALUE{'PD1'}),str2double(PESTcon.VALUE{'PD2'}));
        fprintf(fid,'%g %g %g %g %d\n',str2double(PESTcon.VALUE{'ABSPREDLAM'}),str2double(PESTcon.VALUE{'RELPREDLAM'}),str2double(PESTcon.VALUE{'INITSCHFAC'}),str2double(PESTcon.VALUE{'MULSCHFAC'}),str2double(PESTcon.VALUE{'NSEARCH'}));
        fprintf(fid,'%g %g\n',str2double(PESTcon.VALUE{'ABSPREDSWH'}),str2double(PESTcon.VALUE{'RELPREDSWH'}));
        fprintf(fid,'%d %g %g %d\n',str2double(PESTcon.VALUE{'NPREDNORED'}),str2double(PESTcon.VALUE{'ABSPREDSTP'}),str2double(PESTcon.VALUE{'RELPREDSTP'}),str2double(PESTcon.VALUE{'NPREDSTP'}));
        ...
        fid=fopen([PEST_folder '\' TEMPFLE{1}],'wt');
        fprintf(fid,'ptf #\n');
        fprintf(fid,'PARNAME\tPARVAL\n');
        for i=1:size(PESTpar,1),
            fprintf(fid,'%s\t#%-15s#\n',PESTpar.PARNAME{i},PESTpar.PARNAME{i});
        end
        fclose(fid);
        ...
        for n = 1:length(INSFLE)
            ...
            fid=fopen([PEST_folder '\' INSFLE{n}],'wt');
            fprintf(fid,'pif %%\n');
            for i=1:size(PESTobs_n,1)
                fprintf(fid,'l1 !%s!\n',PESTobs_n.OBSNME{i});
            end
            fclose(fid);
        end
        ...
        fid=fopen([PEST_folder '\' UNCERTAINTYFILE],'wt');
        fprintf(fid,'# Uncertainty file\n');
        fprintf(fid,'\n');
        fprintf(fid,'START STANDARD_DEVIATION\n');
        for i=1:size(PESTpar,1)
            if ~strcmp(PESTpar.PARTRANS{i},'fixed')
                fprintf(fid,'%s\t%g\n',PESTpar.PARNAME{i},PESTpar.STD(i));
            end
        end
        fprintf(fid,'END STANDARD_DEVIATION\n');
        fclose(fid);
