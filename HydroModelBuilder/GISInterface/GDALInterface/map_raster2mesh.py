...
def map_raster_array_to_mesh(hu_raster_path, hu_raster_files, out_path, vtk_out, min_height, max_height):
    ...
    mask_active = np.full((layers, 
                           len(raster_set[hu_raster_files[0]][0]),
                           len(raster_set[hu_raster_files[0]][0][0])), False, dtype=bool)

    mask_thickness = np.full((layers, 
                              len(raster_set[hu_raster_files[0]][0]), 
                              len(raster_set[hu_raster_files[0]][0][0])), False, dtype=bool)

    cumulative_active = np.full((layers, 
                                 len(raster_set[hu_raster_files[0]][0]), 
                                 len(raster_set[hu_raster_files[0]][0][0])), False, dtype=bool)


    #zone_splitting = np.zeros((layers, 
    #                      len(raster_set[hu_raster_files[0]][0]), 
    #                      len(raster_set[hu_raster_files[0]][0][0])))
    ...
    for index, raster in enumerate(hu_raster_files):
        ...
        mask_thickness[index/2] = raster_set[raster][3] - raster_set[hu_raster_files[index+1]][3] < min_height
    ...
    for i in range(index+2,len(hu_raster_files)):                     
        ...
        for j in range(0,i):        
            ...
            proposed_heights = (raster_set[hu_raster_files[i]][3][merge_up_locations] 
                                - (j/2+1)*((raster_set[hu_raster_files[i]][3][merge_up_locations]
                                - raster_set[hu_raster_files[i+1]][3][merge_up_locations])
                                / (i/2+1)))
    ...
    if tester2 == True:
        ...
        for index, raster in enumerate(hu_raster_files):
            if index%2 == 1:continue        
            raster_thickness[index/2] = np.ma.masked_where((zone_matrix[index/2] != index/2+1), raster_set[raster][3] - raster_set[hu_raster_files[index+1]][3])
...
if __name__ == "__main__":
    ...
    hu_raster_files = ["qa_1t_model_grid.bil", "qa_2b_model_grid.bil", "utb_1t_model_grid.bil", "utb_2b_model_grid.bil", "utqa_1t_model_grid.bil", "utqa_2b_model_grid.bil", "utam_1t_model_grid.bil", "utam_2b_model_grid.bil", "utaf_1t_model_grid.bil", "utaf_2b_model_grid.bil", "lta_1t_model_grid.bil", "lta_2b_model_grid.bil", "bse_1t_model_grid.bil", "bse_2b.tif_model_grid.bil"] 
